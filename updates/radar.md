...
## 2026-08-23 — SDK 0.128.0–0.133.0 new; dlt and dbt-databricks unchanged

**dlt** — **1.30.0** (2026-08-11) unchanged. No new releases since the last check.
→ [dlt.md](dlt.md)

**dbt** — **dbt-databricks 1.12.4** (2026-08-12) unchanged. SDK cap still
`databricks-sdk<0.118.0`; repo remains on **0.117.0**. → [dbt.md](dbt.md)

**Databricks** — Six new SDK releases: **0.128.0** (2026-08-13) through **0.133.0** (2026-08-19).
Highlights: full IAM v2 CRUD methods for account + workspace levels (0.128.0); `effective_serverless_compute_id`
on RunTask + IAM v2 list pagination additions (0.129.0–0.130.0); bundle deployment `create_operation()`
removed — breaking (0.131.0); Apps git-source fields, ML `custom_udf`, Pipelines
`effective_serverless_compute_id` in GetPipelineResponse (0.132.0); telemetry feature flags
(0.133.0). All six remain out of reach under the `<0.118.0` adapter cap — repo stays on
